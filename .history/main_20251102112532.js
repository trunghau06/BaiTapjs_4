...
let currentPage = 1;
const itemsPerPage = 10; // render 10 items mỗi lần
...
scrollContainer.addEventListener("scroll", () => {
    ...
    if (scrollTop + clientHeight >= scrollHeight - 200 && !loading && moreDataAvailable) {
        // Hiển thị thông báo đang load với số lượng hiện tại
        const currentTotal = allLoadedData.length;
        loadMoreElement.querySelector('div:last-child').textContent = `Đang tải thêm... (Đã có ${currentTotal} items)`;
        loadMoreElement.style.display = "block";
