...
async function loadMoreData() {
    ...
    try {
        // Tính offset dựa trên số item đã load
        const offset = allLoadedData.length;

        // MockAPI hỗ trợ limit + page, nhưng không offset trực tiếp
        // Cách đơn giản: page = Math.floor(offset / displayCount) + 1, limit = displayCount
        // Vì page * limit sẽ lấy item liên tiếp theo limit → đủ item
        const page = Math.floor(offset / displayCount) + 1;

        const response = await fetch(`${API_URL}?page=${page}&limit=${displayCount}&sortBy=id&order=asc`);
