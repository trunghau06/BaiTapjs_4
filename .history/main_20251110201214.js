...
async function addNewRecordAtStart(record) {
    try {
        ...
        const normalized = normalizeRecord(addedData);
        allLoadedData.unshift(normalized);
...
async function loadMoreData() {
    ...
    try {
        ...
        if (dataList.length === 0) moreDataAvailable = false;
            ...
            const normalized = dataList.map(normalizeRecord);
            allLoadedData = [...allLoadedData, ...normalized];
            appendNewItems(normalized);
            currentPage++;
        }
    } catch (err) {
        console.error(err);
...
function openEditModal(user) {
    ...
    const u = normalizeRecord(user); // chuẩn hóa trước khi fill form

    addRecordForm.name.value     = u.name;
    addRecordForm.company.value  = u.company;
    addRecordForm.genre.value    = u.genre;
    addRecordForm.email.value    = u.email;
    addRecordForm.phone.value    = u.phone;
    addRecordForm.dob.value      = u.dob ? new Date(u.dob).toISOString().slice(0,16) : "";
    addRecordForm.color.value    = u.color;
    addRecordForm.timezone.value = u.timezone;
    addRecordForm.music.value    = u.music;
    addRecordForm.city.value     = u.city;
    addRecordForm.state.value    = u.state;
    addRecordForm.address.value  = u.address;
    addRecordForm.street.value   = u.street;
    addRecordForm.building.value = u.building;
    addRecordForm.zip.value      = u.zip;
    addRecordForm.password.value = u.password;

    avatarPreview.innerHTML = u.avatar
        ? `<img src="${u.avatar}" alt="Preview">`
