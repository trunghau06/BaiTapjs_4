...
async function loadMoreData() {
    ...
    try {
        // Xác định số item xen kẽ
        const dynamicLimit = currentPage % 2 === 1 ? 12 : 24;

        // Sort API trực tiếp
        const response = await fetch(`${API_URL}?page=${currentPage}&limit=${dynamicLimit}&sortBy=id&order=asc`);
        ...
        if (dataList.length === 0) {
            ...
            // Gọi hàm hiển thị dữ liệu mới
            appendNewItems(dataList);

            currentPage++;

            // Ẩn loader khi xong batch đầu tiên
            if (currentPage === 2) {
