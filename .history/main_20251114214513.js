...
const observer = new MutationObserver(mutationsList => {
  for (const mutation of mutationsList) {
    if (mutation.type === 'childList') {
      console.log('Các node con đã được thêm/xóa.');
      // Kiểm tra mutation.addedNodes để xử lý các thẻ mới
    } else if (mutation.type === 'attributes') {
      console.log(`Thuộc tính ${mutation.attributeName} của element đã đổi.`);
    }
  }
});

const config = { attributes: true, childList: true, subtree: true };
// observer.observe(document.body, config); // Bắt đầu quan sát body