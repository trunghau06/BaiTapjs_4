class ResponsiveVirtualView {
    ...
    setupScrollListener() {
    // ticking: cờ để đảm bảo chỉ schedule 1 rAF tại 1 thời điểm
    let ticking = false;

    // dùng passive:true để trình duyệt tối ưu hóa scroll (không bị block bởi handler)
    this.cardsContainer.addEventListener('scroll', () => {
        if (!ticking) {
            ticking = true;
            window.requestAnimationFrame(() => {
                // gọi render được đồng bộ với frame tiếp theo
                this.render();
                this.checkLoadMore();
                ticking = false;
            });
