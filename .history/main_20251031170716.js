class VirtualCards {
    ...
    calculateLayout() {
        ...
        // lấy chiều cao card thực tế nếu đã render ít nhất 1 card
        const firstCard = this.cardsGrid.querySelector('.card');
        if (firstCard) {
            this.cardHeight = firstCard.offsetHeight + gap;
        } else {
            this.cardHeight = 150 + gap; // fallback
        }
    ...
    render() {
        ...
        // update lại cardHeight thực tế mỗi lần render
        const firstCard = this.cardsGrid.querySelector('.card');
        const gap = 20;
        if (firstCard) this.cardHeight = firstCard.offsetHeight + gap;
