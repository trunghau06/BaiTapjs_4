...
async function loadMoreData() {
    ...
    if (!checkMobileView() && currentPage > 1) {
        // Hiển thị loading ngay khi bắt đầu load
        loadMoreElement.style.display = "block";
        loadMoreElement.querySelector('div:last-child').textContent = "Đang tải thêm..";
    } else if (currentPage === 1) {
        loaderElement.style.display = "block"; // loading lần đầu
    }
    ...
    setTimeout(() => {
        ...
        loading = false;
        if (!moreDataAvailable) showCompletionMessage();
    }, 1200);
}
