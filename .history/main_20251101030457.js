class ResponsiveVirtualView {
    constructor() {
        ...
        this.isLoading   = false; // trang thai dang load them du lieu
        this.hasMore     = true; // co con du lieu de load hay khong
        this.currentPage = 1; // trang hien tai khi goi API
        this.rowHeight   = 0; // chieu cao cua 1 hang card hoac row
        
        this.renderedCardIds = new Set(); // luu id cua nhung element da render de xoa khi khong can thiet

        this.cardsContainer = document.getElementById('cardsContainer'); // container chua tat ca cards
        this.cardsSpacer = document.getElementById('cardsSpacer'); // div dung de tao khoang trong virtual scroll
        this.cardsContent = document.getElementById('cardsContent'); // div chua cac element card hien tai
        
        this.tableBody = document.getElementById('tableBody'); // tbody cua table view
        this.tableView = document.getElementById('tableView'); // table view container
        this.cardView = document.getElementById('cardView'); // card view container
        
        this.loader = document.getElementById('loader'); // loader khi load du lieu lan dau
        this.loadingMore = document.getElementById('loadingMore'); // loader khi load them du lieu khi scroll
        
        this.table = document.querySelector('.data-table'); // table hien thi data
        this.fakeScroll = document.getElementById('fakeScroll'); // fake scrollbar de dong bo scroll ngang
        this.fakeWrapper = document.querySelector('.fake-scroll-wrapper'); // wrapper cua fake scrollbar
        
        this.init(); // khoi tao class
    }
    ...
    async init() {
        ...
        this.cardsContainer.style.display = 'block'; // hien thi container cards
        
        window.requestAnimationFrame(() => { 
            this.updateViewMode(); // cap nhat view mode theo man hinh
            
            this.createTemporaryElement(); // tao element tam de do chieu cao
            this.calculateLayout(); // tinh toan layout, rowHeight, visibleRows
            this.removeTemporaryElement(); // xoa element tam

            this.setupScrollListener(); // them scroll listener
            this.setupResizeListener(); // them resize listener
            this.setupHorizontalSync(); // dong bo scroll ngang voi fake scrollbar
            this.render(); // render lan dau
            this.loader.style.display = 'none'; // an loader
        });
    }
    ...
    async loadInitialData() {
        ...
        try {
            ...
            this.data = results.flat(); // ghep tat ca du lieu vao 1 mang
            this.data.sort((a, b) => Number(a.id) - Number(b.id)); // sap xep theo id
            this.currentPage = 6; // cap nhat trang hien tai tiep theo
        } catch (error) {
            console.error("Error loading initial data:", error); // thong bao loi neu co
        }
    }

    async loadData() {
        if (this.isLoading || !this.hasMore) return; // neu dang load hoac khong con du lieu thi thoat
        this.isLoading = true; // dat trang thai dang load
        this.loadingMore.style.display = "block"; // hien thi loading more
        ...
        this.isLoading = false; // ket thuc trang thai load
        this.loadingMore.style.display = 'none'; // an loading more
    }
    ...
    createTemporaryElement() {
        ...
        if (this.data.length > 0 && !targetContainer.querySelector(className)) {
            const sampleElement = this.isMobileView 
                ? this.createCardElement(this.data[0]) // tao card tam de do
                : this.createRowElement(this.data[0]); // tao row tam de do
            
            sampleElement.style.visibility = 'hidden'; // khong hien thi
            sampleElement.id = 'temp-el-for-measurement'; // dat id tam
            targetContainer.appendChild(sampleElement); // them vao container
        }
    }

    removeTemporaryElement() {
        const tempElement = document.getElementById('temp-el-for-measurement'); // lay element tam
        if (tempElement) {
            tempElement.parentElement.removeChild(tempElement); // xoa khoi DOM
        }
    }
    ...
    calculateLayout() {
        const targetContainer = this.isMobileView ? this.cardView : this.tableBody;
        const className = this.isMobileView ? '.card' : '.data-row';
        const gap = this.isMobileView ? 15 : 0; // khoang cach giua cac hang
        ...
        if (firstElement) {
            ...
            this.rowHeight = rect.height + gap; // tinh rowHeight
        } else {
            this.rowHeight = this.isMobileView ? 250 : 80; // mac dinh neu khong co element
        }
    ...
    setupScrollListener() {
        ...
        this.cardsContainer.addEventListener('scroll', () => {
            ...
            scrollTimeout = setTimeout(() => {
                this.render(); // render lai khi scroll
                this.checkLoadMore(); // check co can load them du lieu hay khong
            }, 16);
        });
    }
    ...
    setupResizeListener() {
        ...
        window.addEventListener('resize', () => {
            ...
            resizeTimeout = setTimeout(() => {
                this.updateViewMode(); // cap nhat view mode
                this.createTemporaryElement(); // tao element tam
                this.calculateLayout(); // tinh layout lai
                this.removeTemporaryElement(); // xoa element tam
                this.render(); // render lai
            }, 300);
        });
    ...
    checkLoadMore() {
        ...
        const scrollPercentage = (scrollTop + clientHeight) / scrollHeight; // tinh ty le scroll

        if (scrollPercentage > 0.8 && !this.isLoading && this.hasMore) {
            this.loadData(); // load them du lieu neu scroll den 80%
        }
    }
    ...
    fakeScroll.style.width = table.scrollWidth + 'px'; // set width fake scroll bang table
    ...
    this.cardsContainer.addEventListener('scroll', () => {
        if (!this.isMobileView) {
            fakeWrapper.scrollLeft = this.cardsContainer.scrollLeft; // dong bo nguoc lai
        }
    });
    ...
    render() {
        this.updateViewMode(); // cap nhat view mode truoc khi render
        
        if (this.data.length === 0 || this.rowHeight === 0) {
            this.createTemporaryElement(); // tao element tam
            this.calculateLayout(); // tinh layout
            this.removeTemporaryElement(); // xoa element tam
            if (this.rowHeight === 0) return;
        }

        const scrollTop = this.cardsContainer.scrollTop;
        const startRow = Math.floor(scrollTop / this.rowHeight); // tinh row bat dau render
        ...
        this.renderVisibleElements(); // render cac element hien tai
    }
    ...
    renderVisibleElements() {
        ...
        viewToShow.style.display = 'block'; // hien view hien tai
        viewToHide.style.display = 'none'; // an view khong hien tai

        if (this.fakeWrapper) {
             this.fakeWrapper.style.display = this.isMobileView ? 'none' : 'block'; // an fake scroll khi mobile
        }

        const visibleData = this.data.slice(this.startIndex, this.endIndex); // lay data hien thi
        const newVisibleIds = new Set(visibleData.map(item => Number(item.id))); // lay id cua data hien thi

        const fragment = document.createDocumentFragment(); // tao fragment de append nhanh

        let nodesToRemove = [];
        Array.from(targetContainer.children).forEach(node => {
            const nodeId = Number(node.dataset.id); 
            if (!newVisibleIds.has(nodeId)) { // xoa nhung node khong nam trong visible
    ...
    createRowElement(item) {
        ...
        if (!tempTBody.firstChild || tempTBody.firstChild.nodeName !== 'TR') {
            return document.createElement('tr'); // tao TR trong truong hop khong co
        }
