class VirtualCards 
{
    ...
    checkLoadMore()
    {
        // Lấy vị trí scroll hiện tại (độ cao đã cuộn)
        const scrollTop = this.cardsContainer.scrollTop;

        // Tổng chiều cao có thể cuộn của toàn nội dung
        const scrollHeight = this.cardsContainer.scrollHeight;

        // Chiều cao phần nhìn thấy được
        const clientHeight = this.cardsContainer.clientHeight;

        // Tính phần trăm vị trí cuộn (đã cuộn đến đâu)
        const scrollPercentage = (scrollTop + clientHeight) / scrollHeight;
        
        // Nếu cuộn đến hơn 70% nội dung, và chưa load, và còn dữ liệu
        if (scrollPercentage > 0.7 && !this.isLoading && this.hasMore) {
            // Gọi API để tải thêm dữ liệu (trang kế tiếp)
            this.loadData().then(() => {
            // Sau khi tải xong thì render lại
            this.render();
            });
        }
    }
