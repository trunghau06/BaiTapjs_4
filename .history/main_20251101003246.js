class ResponsiveVirtualView {
    ...
    setupHorizontalSync() { // Cần đảm bảo table đã render để lấy minWidth this.fakeScroll.style.minWidth = this.table.style.minWidth; // Khi scroll thanh giả, cuộn cards container this.fakeWrapper.addEventListener('scroll', () => { this.cardsContainer.scrollLeft = this.fakeWrapper.scrollLeft; }); // Khi cuộn bảng, cập nhật fake scroll this.cardsContainer.addEventListener('scroll', () => { if (!this.isMobileView) { this.fakeWrapper.scrollLeft = this.cardsContainer.scrollLeft; } }); }
    async loadData() {
