class VirtualCards {
    ...
    async init() {
        ...
        // 1. Đo đạc kích thước card/grid chính xác
        ...
        console.log(`\n======================================================
💡 TỔNG QUAN: Loaded ${this.data.length} records. Visible rows: ${this.visibleRows}. Buffer: ${this.bufferRows}
======================================================`);
    }
    ...
    async loadData() {
        ...
        try {
            ...
            if (newData.length === 0) {
                ...
                this.currentPage++;
                this.render(); 
                console.log(`⬆️ LOADED MORE: Thêm ${newData.length} records. Tổng data: ${this.data.length}`);
            }
        } catch (error) {
    ...
    // ... (Các hàm utility: createTemporaryCard, removeTemporaryCard, calculateLayout) ...
    createTemporaryCard() {
    ...
    // ===============================================
    // ✅ DÒNG LOG TRONG HÀM RENDER (TÍNH TOÁN)
    // ===============================================
    ...
    render() {
        ...
        // Dòng log 1: Thể hiện cửa sổ render và offset
        console.log(`
        ------------------------------------------------
        SCROLL: ${Math.round(scrollTop)}px
        WINDOW: ${this.startIndex} - ${this.endIndex} (Total items in window: ${this.endIndex - this.startIndex})
        OFFSET: ${Math.round(offsetY)}px (Spacer H: ${Math.round(totalHeight)}px)
        ------------------------------------------------`);
     ...
     */
    // ===============================================
    // ✅ DÒNG LOG TRONG HÀM RENDERVISIBLECARDS (DOM)
    // ===============================================
    renderVisibleCards() {
        ...
        Array.from(this.cardsGrid.children).forEach(node => {
            ...
            if (!newVisibleIds.has(nodeId)) {
                ...
                cardsRemoved++; // Đếm số card bị xóa
            }
        });
        ...
        visibleData.forEach(item => {
            ...
            if (!cardElement) {
                ...
                cardsAdded++; // Đếm số card được thêm
            }
        ...
        // Dòng log 2: Xác nhận số lượng phần tử DOM thực tế
        console.log(`
        DOM Reconciliation:
            🗑️ REMOVED: ${cardsRemoved} cards
            ➕ ADDED: ${cardsAdded} cards
            🖼️ TOTAL DOM ELEMENTS: ${this.cardsGrid.children.length}
            Total Data Size: ${this.data.length}
        `);
    }

    // ... (Các hàm createCardElement và createCardHTML) ...
