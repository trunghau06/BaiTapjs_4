class VirtualTable {
    ...
    async init() {
        ...
        // SỬA LỖI TIỀM ẨN: Đảm bảo DOM đã sẵn sàng trước khi đo đạc kích thước.
    ...
    // ✅ ĐÃ SỬA LỖI: Đảm bảo createRowHTML trả về chuỗi trước khi gọi trim()
    createRowElement(item) {
        const tempDiv = document.createElement('div');
        
        // Ép kiểu về chuỗi để đảm bảo .trim() không bị lỗi "is not a function"
        tempDiv.innerHTML = String(this.createRowHTML(item)).trim(); 

        // Kiểm tra xem thẻ <tr> có được tạo ra không
        if (!tempDiv.firstChild) {
            console.error("Failed to generate <tr> element for item:", item.id);
