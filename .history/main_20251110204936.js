...
const nextColsBtn = document.getElementById("nextColsBtn"); // Nút Next cột
const prevColsBtn = document.getElementById("prevColsBtn"); // Nút Prev cột
let currentColGroup = 0;
const colsPerPage = 6; // số cột hiển thị mỗi nhóm

// ----------- Hàm hiển thị nhóm cột -----------
function showColumnGroup(groupIndex) {
    ...
    allRows.forEach(row => {
        ...
        cells.forEach((cell, index) => {
            // cột action luôn hiển thị (index 0)
            if (index === 0) {
                cell.style.display = "";
            } else {
                const colGroup = Math.floor((index - 1) / colsPerPage);
                cell.style.display = colGroup === groupIndex ? "" : "none";
            }
        });
    });
}

nextColsBtn?.addEventListener("click", () => {
    const totalCols = tableBodyElement.closest("table").querySelectorAll("th").length - 1;
    const maxGroup = Math.floor((totalCols - 1) / colsPerPage);
    if (currentColGroup < maxGroup) {
        currentColGroup++;
        showColumnGroup(currentColGroup);
    }
});

prevColsBtn?.addEventListener("click", () => {
    if (currentColGroup > 0) {
        currentColGroup--;
        showColumnGroup(currentColGroup);
    }
});

// ----------- Các hàm modal, avatar, add, edit giữ nguyên -----------
...
// ----------- Render table + append items + cột nhóm -----------
...
function appendNewItems(dataList) {
    ...
    showColumnGroup(currentColGroup); // cập nhật cột nhóm sau khi append
}
...
function openEditModal(user) {
    ...
    if (user.dob) addRecordForm.dob.value = formatDobForInput(user.dob);
