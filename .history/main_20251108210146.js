...
async function loadMoreData() {
  ...
  try {
    const response = await fetch(
      `${API_URL}?page=${currentPage}&limit=${itemsPerPage}&sortBy=idNumber&order=asc`
    );
    const newData = await response.json();

    console.log("✅ Dữ liệu trả về:", newData);

    if (newData.length === 0) {
      moreDataAvailable = false;
    } else {
      allLoadedData = [...allLoadedData, ...newData];
      renderTable(allLoadedData); // gọi hàm hiển thị
      currentPage++;
    }
  } catch (error) {
