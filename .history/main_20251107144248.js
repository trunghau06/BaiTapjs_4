...
async function deleteFirstRecord() {
  try {
    if (allLoadedData.length === 0) return; // Nếu không có record thì thôi
    ...
    if (response.ok) {
      ...
      // Xóa record khỏi mảng local
      allLoadedData.shift();

      // Cập nhật lại table / card view
...
switchViewMode();
loadMoreData().then(() => {
    // Bây giờ allLoadedData đã có dữ liệu
    deleteFirstRecord();
});
