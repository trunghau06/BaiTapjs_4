class ResponsiveVirtualView {
    ...
    setupHorizontalSync() {
        // Kiểm tra xem các phần tử cuộn giả có tồn tại không
        if (!this.fakeScroll || !this.fakeWrapper || !this.table) return;

        // Set width fake scroll bằng minWidth của bảng (CSS)
        const tableMinWidth = window.getComputedStyle(this.table).minWidth;
