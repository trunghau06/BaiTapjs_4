...
async function loadMoreData() {
    ...
    try {
        // 1. 🎯 TỐI ĐA HÓA SẮP XẾP API: Yêu cầu API sắp xếp theo ID (tăng dần)
        ...
        if (dataList.length === 0 || allLoadedData.length >= MAX_ITEMS_TOTAL) {
            ...
            // 2. Nối dữ liệu mới vào mảng tổng
            allLoadedData = allLoadedData.concat(dataList);

            // 3. 🎯 KHẮC PHỤC LỖI ID CHUỖI: SẮP XẾP LẠI THEO SỐ NGUYÊN (Client-side Sort)
            // Thao tác này là bắt buộc vì API chỉ sắp xếp chuỗi (string), không phải số học (numeric).
            allLoadedData.sort((a, b) => Number(a.id) - Number(b.id));

            // 4. TÁI RENDER TOÀN BỘ danh sách đã sắp xếp
