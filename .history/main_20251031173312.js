class VirtualCards {
    constructor() {
        ...
        this.currentPage = 1;
        this.cardHeight = 0;
        this.renderedCards = new Map(); // Cache DOM elements (removed - not used anymore)
    ...
    setupResizeListener() {
        ...
        window.addEventListener('resize', () => {
            ...
            resizeTimeout = setTimeout(() => {
                this.renderedCards = new Map(); // Not used anymore
                this.calculateLayout();
                this.render();
            }, 300);
        });
    ...
    render() {
        ...
        // Update cardHeight từ DOM thực tế
        const firstCard = this.cardsGrid.querySelector('.card');
        if (firstCard) {
            ...
            this.cardHeight = 450; // Fallback estimate
        }
        ...
        // Tính startIndex và endIndex cẩn thận
        this.startIndex = adjustedStartRow * this.cardsPerRow;
        
        // Tính endRow và đảm bảo render đủ cards
        ...
        // KHÔNG dùng transform - để cards tự nhiên trong flow
        // Thay vào đó dùng padding-top để tạo offset
    ...
    renderVisibleCards() {
        ...
        // Render tất cả cards visible bằng innerHTML (đơn giản nhất, đảm bảo không bỏ sót)
        this.cardsGrid.innerHTML = visibleData
            .map(item => this.createCardHTML(item))
            .join('');

        console.log(`🎨 Rendered ${visibleData.length} cards (index ${this.startIndex}-${this.endIndex}, IDs: ${visibleData[0]?.id} → ${visibleData[visibleData.length-1]?.id})`);
    }
