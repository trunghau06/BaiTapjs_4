...
async function loadMoreData() {
    ...
    try {
        // Thử yêu cầu API sort với tham số khác (orderBy=id&direction=asc)
        const response = await fetch(`${API_URL}?page=${currentPage}&limit=${itemsPerPage}&orderBy=id&direction=asc`);
        ...
        if (dataList.length === 0) {
            ...
            // Kiểm tra nếu API đã sort đúng số; nếu không, sort client-side
