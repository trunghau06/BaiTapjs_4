...
async function loadMoreData() {
    ...
    try {
        // Thêm tham số sắp xếp vào URL (sortBy=id&order=asc để sắp xếp theo ID tăng dần)
        ...
        if (dataList.length === 0) {
            ...
            // Loại bỏ sắp xếp phía client vì API đã xử lý
            // allLoadedData.sort((a, b) => parseInt(a.id) - parseInt(b.id)); // Đã xóa
