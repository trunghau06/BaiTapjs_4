class VirtualTable {
    constructor() {
        ...
        this.hasMore = true;
        this.currentPage = 1;
        this.rowHeight = 0; // Chiều cao của một hàng (row)
        ...
        this.tableBody = document.getElementById('tableBody'); // body của bảng
    ...
    async loadData() {
        ...
        try {
            ...
            if (newData.length === 0) {
                ...
                // Thêm dữ liệu mới và sắp xếp
                this.data = [...this.data, ...newData];
                this.data.sort((a, b) => Number(a.id) - Number(b.id)); 
                this.currentPage++;
                // Gọi render() để cập nhật virtual scrolling
                this.render(); 
            }
        } catch (error) {
    ...
    removeTemporaryRow() {
        ...
        if (tempRow) {
            this.tableBody.removeChild(tempRow); // Lỗi "tempCard" đã được sửa thành "tempRow"
        }
    }
    ...
    calculateLayout() {
        ...
        if (firstRow) {
            ...
            this.rowHeight = 80; // Giá trị mặc định
        }

        if (this.rowHeight > 0) {
            const containerHeight = this.cardsContainer.clientHeight;
            // Tính số lượng hàng hiển thị + 1 hàng dự phòng
    ...
    checkLoadMore() {
        ...
        // Kiểm tra xem đã cuộn đến 80% cuối cùng chưa
    ...
    renderVisibleRows() {
        ...
        visibleData.forEach(item => {
            ...
            // Thêm vào fragment (tự động sắp xếp theo thứ tự data)
