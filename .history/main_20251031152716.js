class VirtualCards 
{
    ...
    setupResizeListener() 
    {
        ...
        // Khi cửa sổ trình duyệt thay đổi kích thước
  window.addEventListener('resize', () => {
    // Xóa timeout cũ để tránh gọi quá nhiều lần
    clearTimeout(resizeTimeout);

    // Sau 200ms kể từ lần resize cuối cùng, mới tính lại layout
    resizeTimeout = setTimeout(() => {
      this.calculateLayout(); // Cập nhật lại số cột hiển thị
      this.render();          // Render lại danh sách theo layout mới
    }, 200);
  });
    }
}