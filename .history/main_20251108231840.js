...
addRecordForm.addEventListener("submit", async (e) => {
    ...
    // Nếu edit, giữ avatar cũ mặc định; nếu add mới, bắt buộc phải có file
    let avatarUrl = editingUser?.avatar || ""; 
    const avatarFile = formData.get("avatarFile");

    if (!isEditMode) {
        // ADD MỚI: phải chọn file
        ...
        // EDIT: chỉ resize nếu có file mới, không có thì giữ avatar cũ
        if (avatarFile && avatarFile.size > 0) {
            avatarUrl = await resizeImage(avatarFile, 80, 80);
        }
    }
    ...
    const recordData = {
        avatar: avatarUrl, // avatar cũ hoặc mới
...
const submitBtn = addRecordForm.querySelector(".btn-submit"); // lấy nút submit
...
addRecordBtn.addEventListener("click", () => {
    ...
    // đổi chữ nút submit về "Thêm Record"
...
function openEditModal(user) {
    ...
    // đổi chữ nút submit khi edit
    submitBtn.textContent = "Chỉnh sửa";
}

// Ngoài ra, khi đóng modal reset nút submit cũng nên đặt lại:
closeModalBtn.addEventListener("click", () => {
    addRecordModal.style.display = "none";
    addRecordForm.reset();
    resetAvatarPreview();
