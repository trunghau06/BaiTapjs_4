...
let loadCount = 0; // theo dõi số lần load để xen kẽ
const firstBatch = 12;
const secondBatch = 24;
...
async function loadMoreData() {
    ...
    const skipCount = allLoadedData.length; // số item đã load → dùng để skip
    ...
    try {
        const response = await fetch(`${API_URL}?limit=${itemsToLoad}&skip=${skipCount}&sortBy=id&order=asc`);
