...
// Hàm kiểm tra xem dữ liệu có sắp xếp đúng số không
function isDataSortedNumerically(dataList) {
    for (let i = 1; i < dataList.length; i++) {
        if (parseInt(dataList[i - 1].id) > parseInt(dataList[i].id)) {
            return false; // Không sắp xếp đúng
        }
    }
    return true; // Đã sắp xếp đúng
}
...
async function loadMoreData() {
    ...
    try {
        // Thử yêu cầu API sort với tham số khác (sort=id&order=asc)
        const response = await fetch(`${API_URL}?page=${currentPage}&limit=${itemsPerPage}&sort=id&order=asc`);
        ...
        if (dataList.length === 0) {
            ...
            // Kiểm tra nếu API đã sort đúng số; nếu không, sort client-side với padding
            if (!isDataSortedNumerically(allLoadedData)) {
                allLoadedData.sort((a, b) => {
                    const paddedA = (a.id || '').padStart(3, '0');
                    const paddedB = (b.id || '').padStart(3, '0');
                    return paddedA.localeCompare(paddedB);
                });
            }

            // Xóa dữ liệu cũ trong DOM và append lại từ allLoadedData đã sort
            clearAndReappendData();
...
// Hàm xóa dữ liệu cũ và append lại từ allLoadedData đã sort
function clearAndReappendData() {
    // Xóa table và card cũ
    tableBodyElement.innerHTML = '';
    cardViewElement.innerHTML = '';

    // Append lại từ allLoadedData đã sort
