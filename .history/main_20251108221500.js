...
function addNewRecordAtStart(record) {
    // Tạo id tạm thời (giả lập auto-increment)
    const maxId = allLoadedData.length > 0 ? Math.max(...allLoadedData.map(r => Number(r.id))) : 0;
    record.id = maxId + 1;

    allLoadedData.unshift(record); // thêm record mới vào đầu mảng

    renderTable(allLoadedData); // render table + card view
