class VirtualCards {
    constructor() {
        ...
        this.bufferRows = 2; // So luong hang dem buffer o tren va duoi
    ...
    async init() {
        ...
        // THIẾT LẬP BAN ĐẦU CHO 8 CARD
        if (this.cardsPerRow > 0) { // Kiem tra cardsPerRow da tinh toan
            this.visibleRows = Math.ceil(8 / this.cardsPerRow); // Tinh so hang can de chua 8 card
            this.bufferRows = 0; // Dat buffer bang 0 de khong vuot qua 8 card ban dau
        }
        // KẾT THÚC THIẾT LẬP BAN ĐẦU CHO 8 CARD
    ...
    async loadInitialData() {
        ...
        for (let page = 1; page <= 5; page++) { // Lap qua 5 trang de tai 100 records
            promises.push(
        ...
        try {
            const results = await Promise.all(promises); // Doi tat ca cac promise hoan thanh
            this.data = results.flat(); // Gop tat ca du lieu tu cac trang vao mot mang
            this.data.sort((a, b) => Number(a.id) - Number(b.id)); // Sap xep data theo ID
            this.currentPage = 6; // Dat trang hien tai la 6 de lan fetch tiep theo bat dau tu 6
        } catch (error) {
            console.error("Error loading initial data:", error); // Xu ly loi tai data
        }
    }

    async loadData() {
        if (this.isLoading || !this.hasMore) return; // Thoat neu dang tai hoac khong con data
        this.isLoading = true; // Dat trang thai dang tai
        this.loadingMore.style.display = "block"; // Hien thi spinner tai them

        try {
            const response = await fetch( // Goi API voi trang hien tai
                `https://671891927fc4c5ff8f49fcac.mockapi.io/v2?page=${this.currentPage}&limit=20`
            );
            const newData = await response.json(); // Lay data moi

            if (newData.length === 0) { // Kiem tra neu API khong tra ve data nua
                this.hasMore = false; // Dat trang thai khong con data
            } else {
                this.data = [...this.data, ...newData]; // Them data moi vao mang
                this.data.sort((a, b) => Number(a.id) - Number(b.id)); // Sap xep lai data
                this.currentPage++; // Tang so trang de tai lan toi
                this.render(); // Render lai de cap nhat view va spacer
            }
        } catch (error) {
            console.error("Error loading more data:", error); // Xu ly loi tai them data
        }

        this.isLoading = false; // Ket thuc trang thai tai
        this.loadingMore.style.display = 'none'; // An spinner tai them
    }
    
    createTemporaryCard() {
        if (this.data.length > 0 && !this.cardsGrid.querySelector('.card')) { // Chi tao neu co data va chua co card nao
             const sampleCard = this.createCardElement(this.data[0]); // Tao card dau tien
             sampleCard.style.visibility = 'hidden'; // An card nhung van giu de do kich thuoc
             sampleCard.id = 'temp-card-for-measurement'; // Gan ID de de dang xoa
             this.cardsGrid.appendChild(sampleCard); // Chen vao DOM
        }
    }
