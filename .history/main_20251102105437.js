...
function updateViewMode() {
    if (isMobileView()) {
        ...
        if (fakeScrollWrapper) fakeScrollWrapper.style.display = 'none';
    } else {
        tableViewEl.style.display = 'block';
        cardViewEl.style.display = 'none';
        if (fakeScrollWrapper) fakeScrollWrapper.style.display = 'block';
    }
}
...
// -------------------- Giữ DOM tối đa --------
function trimDom(containerEl) {
    while (containerEl.children.length > maxDomItems) {
        containerEl.removeChild(containerEl.children[0]); // remove item cũ từ trên
    }
}
