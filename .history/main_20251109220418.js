...
// ====== Điều khiển nhóm cột (hiển thị 6 cột chính, chuyển qua nhóm kế tiếp) ======
const thElements = document.querySelectorAll("thead th");
let currentGroupIndex = 0;
const groupSize = 6; // hiển thị 6 cột (ngoại trừ cột Delete/Edit đầu tiên)

const prevBtn = document.getElementById("prevColumn");
const nextBtn = document.getElementById("nextColumn");

// Ẩn/hiện cột theo nhóm
function updateVisibleColumns() {
  const totalCols = thElements.length;
  thElements.forEach((th, i) => {
    const isActionCol = i === 0; // Cột đầu tiên (nút hành động)
    const inGroup =
      i >= currentGroupIndex * groupSize + 1 &&
      i < (currentGroupIndex + 1) * groupSize + 1;
    th.style.display = isActionCol || inGroup ? "" : "none";
  });

  // Ẩn/hiện td tương ứng trong tbody
  document.querySelectorAll("#tableBody tr").forEach((row) => {
    row.querySelectorAll("td").forEach((td, i) => {
      const isActionCol = i === 0;
      const inGroup =
        i >= currentGroupIndex * groupSize + 1 &&
        i < (currentGroupIndex + 1) * groupSize + 1;
      td.style.display = isActionCol || inGroup ? "" : "none";
    });
  });
}

// Gắn sự kiện cho nút chuyển
if (prevBtn && nextBtn) {
  ...
  nextBtn.addEventListener("click", () => {
    const maxIndex = Math.ceil((thElements.length - 1) / groupSize) - 1;
...
// Khởi tạo khi load bảng xong
setTimeout(updateVisibleColumns, 1000);
