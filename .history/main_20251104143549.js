...
let nextBatchSize = itemsPerPage; // batch đầu tiên = 10
let doubleNext = true; 
let offset = 0; 
...
async function loadMoreData() {
    ...
    try {
        // Dùng page=1 luôn, nhưng tăng limit và lấy slice
        const response = await fetch(`${API_URL}?page=1&limit=${offset + limit}&sortBy=id&order=asc`);
        const allData = await response.json();
        
        // Chỉ lấy phần mới từ offset
        const dataList = allData.slice(offset, offset + limit);
        ...
        if (dataList.length === 0) {
            ...
            console.log(`✅ Hiển thị ${dataList.length} items (Tổng: ${allLoadedData.length}/100)`);

            // Cập nhật batch tiếp theo: 10 → 20 → 10 → 20 …
    ...
    if (!moreDataAvailable || allLoadedData.length >= 100) {
        ...
        loading = false;
        console.log(`🎉 Hoàn tất ${allLoadedData.length} items`);
    } else {
        setTimeout(() => {
            loadMoreElement.style.display = "none"; 
            loading = false;
