...
let totalRecords        = 0;
let renderedElements    = []; // Giữ lại mảng này cho các TR
let renderedCardElements = []; // Thêm mảng này cho Card View
let fullDataStore       = {};
let lastScrollTop       = 0;
...
async function fetchInitialData() {
    try {
        ...
        for (let i = 0; i < RENDER_WINDOW; i++) {
            ...
            // Tạm thời tạo card để đảm bảo mảng renderedCardElements có đủ chỗ
            const card = createCardElement(fullDataStore[i]); 
            cardViewElement.appendChild(card);
            renderedCardElements.push(card);
        }
...
function updateVirtualView() {
    ...
    // Logic render cho TABLE VIEW
    if (tableSection.style.display === 'block') {
        const container = tableBodyElement;
        ...
        for (let i = 0; i < RENDER_WINDOW; i++) {
            ...
            if (dataIndex < totalRecords) {
                ...
                const offset = dataIndex * ROW_HEIGHT;
    ...
    // Logic render cho CARD VIEW (Nếu bạn cần Card View Virtualization)
    /*
    else if (cardSection.style.display === 'flex') {
        // ... (Logic tương tự như trên cho renderedCardElements)
    }
    */
}
