...
const observer = new IntersectionObserver((entries, observer) => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      // 1. Element đã đi vào viewport
      const img = entry.target;
      const src = img.getAttribute('data-src'); // Lấy đường dẫn thực
      img.src = src; // Gán để bắt đầu tải
      observer.unobserve(img); // 2. Ngừng quan sát sau khi tải
    }
  });
}, { threshold: 0.01 }); // Kích hoạt khi chỉ 1% element xuất hiện
