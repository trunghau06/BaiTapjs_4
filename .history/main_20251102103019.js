class ResponsiveVirtualView {
    ...
    async loadInitialData() {
        try {
            const response = await fetch(
                // Thay đổi limit=20 thành limit=15
    ...
    async loadData() {
        ...
        this.isLoading = true;
        this.loadingMore.style.display = "block"; // hiện chữ "Đang load..."

        try {
            const response = await fetch(
                `https://671891927fc4c5ff8f49fcac.mockapi.io/v2?page=${this.currentPage}&limit=8`
            );
        ...
        this.isLoading = false;
        this.loadingMore.style.display = "none"; // ẩn chữ load
    }
    ...
    checkLoadMore() {
        ...
        // scroll tới đáy container
        if (Math.ceil(scrollTop + clientHeight) >= scrollHeight && !this.isLoading && this.hasMore) {
            this.loadData();
        }
    }
