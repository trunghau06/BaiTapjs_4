class VirtualCards {
    ...
    async loadInitialData() {
        const promises = [];
        for (let page = 1; page <= 5; page++) { // load 100 cards
            promises.push(fetch(`https://671891927fc4c5ff8f49fcac.mockapi.io/v2?page=${page}&limit=20`)
                .then(res => res.json()));
        }
        try {
    ...
    calculateLayout() {
        ...
        if (firstCard) {
            ...
            this.cardHeight = firstCard.offsetHeight + parseInt(style.marginBottom) || 400;
        } else {
