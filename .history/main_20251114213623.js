...
function debounce(func, delay) {
  let timerId; // Biến được closure ghi nhớ

  return function(...args) {
    clearTimeout(timerId); // 1. Xóa timer cũ nếu có
    timerId = setTimeout(() => { // 2. Thiết lập timer mới
      func.apply(this, args); // 3. Chạy hàm sau delay
    }, delay);
  };
}

// Ví dụ: Chỉ gọi hàm sau 500ms người dùng ngừng gõ
const handleSearch = debounce(searchTerm => {
  console.log('Đang tìm kiếm:', searchTerm);
}, 500);

// window.addEventListener('keyup', (e) => handleSearch(e.target.value));