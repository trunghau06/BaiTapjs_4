...
let loadCount = 0;
const displayBatches = [10, 20]; // Xen kẽ 10 và 20
...
async function loadMoreData() {
    ...
    // Lấy số lượng items theo batch xen kẽ
    ...
    try {
        ...
        if (!Array.isArray(dataList) || dataList.length === 0) {
            ...
            // Kiểm tra nếu nhận được ít hơn yêu cầu = hết data
            if (dataList.length < displayCount) {
                moreDataAvailable = false;
            }
    ...
    if (!moreDataAvailable) {
        console.log(`✅ Đã load hết ${allLoadedData.length} items`);
    } else {
        // Tự động load batch tiếp theo
