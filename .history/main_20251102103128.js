class ResponsiveVirtualView {
    ...
    async loadInitialData() {
        try {
            const response = await fetch(
                `https://671891927fc4c5ff8f49fcac.mockapi.io/v2?page=1&limit=15`
            );
            const initialData = await response.json();
            
            this.data = initialData;
            this.data.sort((a, b) => Number(a.id) - Number(b.id)); 
            this.currentPage = 2; // Bắt đầu tải thêm từ trang 2
        } catch (error) {
    ...
    async loadData() {
        ...
        try {
            const response = await fetch(
                `https://671891927fc4c5ff8f49fcac.mockapi.io/v2?page=${this.currentPage}&limit=15`
            );
    ...
    checkLoadMore() {
        ...
        // 1. Tính tổng chiều cao của TẤT CẢ dữ liệu đã được tải vào bộ nhớ (this.data.length).
        const totalLoadedHeight = this.data.length * this.rowHeight;
        
        // 2. Đặt Ngưỡng Kích hoạt Tải (Threshold)
        // Kích hoạt khi đáy viewport còn cách cuối danh sách khoảng 3 lần chiều cao của 1 hàng.
        const thresholdRows = 3; 
        const thresholdHeight = Math.max(0, totalLoadedHeight - (thresholdRows * this.rowHeight));
        
        // 3. KIỂM TRA ĐIỀU KIỆN
        // Kích hoạt load nếu ĐÁY Viewport (scrollTop + clientHeight) vượt qua Ngưỡng Tải.
        if (scrollTop + clientHeight >= thresholdHeight && !this.isLoading && this.hasMore) {
            this.loadData();
        }
    }
    ...
    createRowHTML(item) {
        ...
        return `
            <tr class="data-row" data-id="${item.id}" style="background-color: ${bgColor};">
                <td>${item.id || 'N/A'}</td>
    ...
    createCardHTML(item) {
        ...
        const bgColor    = item.color || '#fff'; // màu nền từ API

        return `
            <div class="card" data-id="${item.id}" style="background-color: ${bgColor};">
