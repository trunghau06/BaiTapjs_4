...
addRecordForm.addEventListener("submit", async (e) => {
    ...
    let avatarUrl = editingUser?.avatar || ""; // mặc định giữ avatar cũ nếu edit

    const avatarFile = formData.get("avatarFile");

    if (!isEditMode) {
        // ADD MỚI: bắt phải chọn avatar
        ...
        // EDIT: nếu có file mới thì resize, không có thì giữ avatar cũ
        if (avatarFile && avatarFile.size > 0) {
            avatarUrl = await resizeImage(avatarFile, 80, 80);
        }
    }
