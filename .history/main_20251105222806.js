...
async function deleteLastRecord() {
  try {
    ...
    // Sắp xếp theo id tăng dần (dù là số hay chuỗi)
    const sortedData = data.sort((a, b) => Number(a.id) - Number(b.id));

    // Lấy id của record cuối cùng
    const lastRecordId = sortedData[sortedData.length - 1].id;

    // Xóa record đó
    const deleteRes = await fetch(`${API_URL}/${lastRecordId}`, {
      method: "DELETE",
    });

    if (!deleteRes.ok) {
      console.error("Lỗi khi xóa record:", await deleteRes.text());
      return;
    }

    console.log(`✅ Đã xóa record có id: ${lastRecordId}`);
  } catch (error) {
...
async function deleteLastRecord() {
  try {
    const response = await fetch(`${API_URL}?sortBy=id&order=asc`);
    const data = await response.json();
    ...
    // Lấy id cuối cùng trong danh sách thật
    const lastRecordId = data[data.length - 1].id;
...
switchViewMode();
(async () => {
  await deleteLastRecord(); // Xóa record cuối cùng đang tồn tại thật
})();
