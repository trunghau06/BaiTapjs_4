class VirtualCards {
    constructor() {
        ...
        this.cardsPerRow = 0; 
        this.visibleRows = 0;
        this.bufferRows = 2; // số hàng buffer trước và sau
        this.startIndex = 0;
        this.endIndex = 0;
        ...
        this.renderedCardIds = new Set(); // lưu id card đang render
    ...
    async loadInitialData() {
        // load 5 page ban dau (20 record/page)
        const promises = [];
    ...
    renderVisibleCards() {
        ...
        // xoa node khong nam trong visible
        ...
        visibleData.forEach(item => {
            if (!this.renderedCardIds.has(item.id)) {
                const card = this.createCardElement(item);
                this.cardsGrid.appendChild(card);
                this.renderedCardIds.add(item.id);
            }
